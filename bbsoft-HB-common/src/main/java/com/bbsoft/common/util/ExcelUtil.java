...
public class ExcelUtil {
	...
	public static <T> void listToExcel(List<T> list, LinkedHashMap<String, String> fieldMap, String sheetName,
		...
		if (list.size() == 0 || list == null) {
			throw new ServiceException(MsgeData.SYSTEM_10111.getCode());
		}
		...
		try {
			...
			logger.debug(MsgeData.SYSTEM_10112.getMsg() + "{}", e.toString());
	...
	public static <T> List<T> excelToList(InputStream in, String sheetName, Class<T> entityClass,
		...
		try {
			...
			// 如果Excel中没有数据则提示错误
			if (realRows <= 1) {
				throw new ServiceException(MsgeData.SYSTEM_10113.getCode());
			}
			...
			// 如果有列名不存在，则抛出异常，提示错误
			if (!isExist) {
				throw new ServiceException(MsgeData.SYSTEM_10114.getCode());
			}
			...
			for (int i = 1; i < realRows; i++) {
				...
				if (nullCols == uniqueFields.length) {
					throw new ServiceException(MsgeData.SYSTEM_10115.getCode());
				}
			}
			...
			logger.debug(MsgeData.SYSTEM_10116.getMsg() + "{}", e.toString());
			throw new ServiceException(MsgeData.SYSTEM_10116.getCode());
		}
		return resultList;
	}
	...
	private static Object getFieldValueByName(String fieldName, Object o) throws Exception {
		...
		if (field != null) {
			...
			logger.debug("类{}， 字段名{}不存在", o.getClass().getSimpleName(), fieldName);
	...
	private static void setFieldValueByName(String fieldName, Object fieldValue, Object o) throws Exception {
		...
		if (field != null) {
			...
			if (String.class == fieldType) {
				...
				field.set(o, DateUtil.strToDate(fieldValue.toString(), DateUtil.GLOBAL_TIME_PATTERN));
			} else {
